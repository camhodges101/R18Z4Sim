...
public:
    ...
    piston(float TDC_Angle) {
        ...
        bore = 81e-3;
        rodlength = 157.5e-3;
        headvolume = 0.0000468;
        position = 0;
        angleOffset = TDC_Angle;
        cyclestate = 0;
        topPosition = (0.5 * stroke) + rodlength;
        bottomPosition = (0.5 * stroke) - rodlength;
        ATMpress = 101e+3;
    }
    ...
    void update(float* crankAngle) {
        ...
        */
        float x1, y1, x2, y2;
        x1 = 0.5 * stroke * sin(*crankAngle - angleOffset);
        y1 = 0.5 * stroke * cos(*crankAngle - angleOffset);
        x2 = 0;
...
public:
    ...
    int numCyl;
private:
    float offsetAngles[4] = { 0, 2 * pi, pi, 3 * pi };
    piston* cylList[4];
    int k=0;
